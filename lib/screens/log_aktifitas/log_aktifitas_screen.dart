...
class _LogAktifitasScreenState extends State<LogAktifitasScreen> {
  ...
  void _applyFilter({String? keyword, String? aktor, DateTime? start, DateTime? end}) {
    setState(() {
      _filterAktor = aktor;
      _filterStart = start;
      _filterEnd = end;
      ...
      _filteredItems = _allItems.where((item) {
        ...
        if (q.isNotEmpty) {
          final matches = (item['deskripsi'] ?? '').toLowerCase().contains(q) || (item['aktor'] ?? '').toLowerCase().contains(q) || (item['tanggal'] ?? '').toLowerCase().contains(q);
            ...
            child: ConstrainedBox(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    const Text('Filter Log Aktifitas', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 12),
                    const Text('Kata kunci (deskripsi / aktor / tanggal)'),
                    TextField(
                      controller: TextEditingController.fromValue(TextEditingValue(text: tempKeyword)),
                    ...
                    DropdownButtonFormField<String>(
                      value: tempAktor,
                      items: <String>{..._allItems.map((e) => e['aktor'] ?? '')}.map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(),
                      onChanged: (v) => setStateDialog(() => tempAktor = v),
                      decoration: const InputDecoration(hintText: '-- semua aktor --'),
                    ),
                    const SizedBox(height: 12),
                    Row(
                      children: [
                            ...
                            children: [
                              ...
                              InkWell(
                                onTap: () async {
                                  ...
                                  if (picked != null) setStateDialog(() => tempStart = picked);
                                },
                                child: Container(
                                  padding: const EdgeInsets.symmetric(vertical: 10, horizontal: 12),
                                  decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(6)),
                                  child: Text(tempStart != null ? '${tempStart!.day}/${tempStart!.month}/${tempStart!.year}' : '--/--/----'),
                                ),
                              ),
                            ],
                          ),
                            ...
                            children: [
                              ...
                              InkWell(
                                onTap: () async {
                                  ...
                                  if (picked != null) setStateDialog(() => tempEnd = picked);
                                },
                                child: Container(
                                  padding: const EdgeInsets.symmetric(vertical: 10, horizontal: 12),
                                  decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(6)),
                                  child: Text(tempEnd != null ? '${tempEnd!.day}/${tempEnd!.month}/${tempEnd!.year}' : '--/--/----'),
                                ),
                              ),
                            ],
                          ),
                    ...
                    Row(
                      ...
                      children: [
                        TextButton(onPressed: () { _resetFilter(); Navigator.of(context).pop(); }, child: const Text('Reset')),
                        const SizedBox(width: 12),
                        ElevatedButton(
                          onPressed: () {
                            _applyFilter(keyword: tempKeyword, aktor: tempAktor, start: tempStart, end: tempEnd);
                ...
                children: [
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Log Aktifitas', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
                            const SizedBox(height: 4),
                            Text('${_filteredItems.length} entri', style: TextStyle(color: Colors.grey.shade600, fontSize: 13)),
                          ],
                        ),
                      ),
                      const SizedBox(width: 12),
                      ...
                      child: Column(
                        children: [
                          ...
                          Container(
                            ...
                            child: Row(
                              children: const [
                                Expanded(flex: 1, child: Text('NO', style: TextStyle(color: Colors.black54, fontSize: 13, fontWeight: FontWeight.w700))),
                                Expanded(flex: 3, child: Text('TANGGAL', style: TextStyle(color: Colors.black54, fontSize: 13, fontWeight: FontWeight.w700))),
                                Expanded(flex: 6, child: Text('DESKRIPSI', style: TextStyle(color: Colors.black54, fontSize: 13, fontWeight: FontWeight.w700))),
                                Expanded(flex: 3, child: Text('AKTOR', textAlign: TextAlign.left, style: TextStyle(color: Colors.black54, fontSize: 13, fontWeight: FontWeight.w700))),
                                Expanded(flex: 1, child: Text('AKSI', textAlign: TextAlign.center, style: TextStyle(color: Colors.black54, fontSize: 13, fontWeight: FontWeight.w700))),
                              ],
                            ),
                          ),
                            ...
                            child: _filteredItems.isEmpty
                                    ...
                                    child: Padding(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          const SizedBox(height: 12),
                                          Text('Tidak ada log yang sesuai filter', style: TextStyle(color: Colors.grey.shade600)),
                                        ],
                                      ),
                                    ),
                                  )
                                : ListView.separated(
                                    ...
                                    itemBuilder: (context, index) {
                                      ...
                                      final rowIndex = index + 1 + (_currentPage - 1) * _rowsPerPage;
                                      return Container(
                                        padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 12),
                                        child: Row(
                                          crossAxisAlignment: CrossAxisAlignment.center,
                                          children: [
                                            Expanded(flex: 1, child: Text(rowIndex.toString(), style: const TextStyle(color: Colors.black87))),
                                            Expanded(flex: 3, child: Text(item['tanggal'] ?? '', style: const TextStyle(color: Colors.black87))),
                                            Expanded(flex: 6, child: Text(item['deskripsi'] ?? '', style: const TextStyle(color: Colors.black87), overflow: TextOverflow.ellipsis)),
                                            Expanded(flex: 3, child: Text(item['aktor'] ?? '', style: const TextStyle(color: Colors.black87))),
                                            Expanded(
                                              flex: 1,
                                              child: Align(
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      for (var p = 1; p <= (_pageCount <= 5 ? _pageCount : 5); p++)
                        Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 4.0),
                          child: OutlinedButton(
                            onPressed: () => _goToPage(p),
                            style: OutlinedButton.styleFrom(
                              backgroundColor: _currentPage == p ? const Color(0xFF2B3A66) : Colors.transparent,
