...
**Description:**  
This professional interface, titled the **Citizen Aspirations List**, is built upon a center-aligned Card container to ensure a clean, modern aesthetic and consistent responsiveness across all devices. The primary feature is the data table, which employs flexible layout principles to maintain a balanced column structure, distributing space proportionally among key data points such as Sender, Title, Status, and Date. Crucially, the management capabilities are highlighted by the **Filters** button in the top-right corner, allowing users to efficiently segment and refine the large dataset based on critical criteria like the item's current Status **Diterima/Approved, Pending, or Ditolak/Rejected**. Visual comprehension is enhanced by the**Status Chip** feature in each row, which uses color-coding (green, yellow, red) for instant, scannable feedback on the aspiration's state. Finally, the Aksi (Action) column provides direct item management via a three-dot icon, which triggers a contextual menu to perform immediate actions such as viewing the **Detail**, initiating an **Edit process**, or confirming a **Delete** action.
